...
private static URI uri = UriBuilder.fromUri("http://localhost:8080/playlist-ws/rest/users/").build();
	...
	public void getUserInfoFail() {
		Client client = ClientBuilder.newClient();
		Response response = client.target(uri+"0").request(MediaType.APPLICATION_XML).get();
	...
	public void getUserMusicFail() {
		Client client = ClientBuilder.newClient();
		Response response = client.target(uri+"989898/songs").request(MediaType.APPLICATION_XML).get();
		assertEquals(Response.Status.NOT_FOUND, response.getStatusInfo());
		client.close();
	}
