...
public class UserResource {
	...
	public Total getMessage() {
		...
		Total totalUsers = new Total(); 
		totalUsers.setTotal(converterCdiBean.getUserList().getUser().size());
		return totalUsers;
	}
	...
	public Response deleteUserSong(@PathParam("userId") int idUser,
			@PathParam("songId") int idSong) {
	...
	public Response removeUser(@PathParam("userId") int id) {
		...
		converterCdiBean.removeUser(id);
		return Response.ok().build();
	}
	...
	public Total getLoggedNumber() {
		log.info("Getting the number of logged users.");
		Total totalLogged = new Total();
		totalLogged.setTotal(converterCdiBean.getLoggedUsers()
				.getUser().size());
		return totalLogged;
	}
}
